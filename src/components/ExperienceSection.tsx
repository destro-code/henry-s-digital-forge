    ...
    <section id="experience" className="py-24 md:py-32 px-6">
      <div ref={ref} className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-bold text-foreground">Professional <span className="text-primary text-glow">Experience</span></h2>
          <p className="text-muted-foreground max-w-2xl mt-5 leading-relaxed">Professional work and ongoing independent development across frontend and full-stack web applications.</p>
        </motion.div>
        ...
        <div className="relative">
          ...
          <div className="space-y-12">
            {experiences.map((exp, i) => (
              <motion.div key={`${exp.company}-${exp.period}`} initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.15 }} className="relative pl-12 md:pl-16">
                ...
                <div className="glass rounded-xl p-6 md:p-8 hover:box-glow transition-all duration-300">
                  ...
                  <ul className="space-y-2">
                    {exp.points.map((point, j) => <li key={j} className="text-muted-foreground text-sm leading-relaxed flex gap-2"><span className="text-primary mt-1.5 shrink-0">▸</span>{point}</li>)}
                  </ul>
                </div>
              </motion.div>
            ))}
